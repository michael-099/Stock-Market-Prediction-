...
def prepare_data(tesla_data, time_steps=50):
    scaled_data = scaler.fit_transform(tesla_data[['Close']].values)
    model_inputs = scaled_data[-time_steps:]  
...
def predict_for_date(start_date, end_date):
    ...
    predicted_price_scaled = model.predict(model_inputs)

    predicted_price = scaler.inverse_transform(np.concatenate((predicted_price_scaled, np.zeros_like(predicted_price_scaled)), axis=1))[:, 0]
